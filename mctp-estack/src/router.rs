...
/// Active listeners also take up a `MAX_RECEIVERS` slot.
///
/// Must be power of 2.
...
struct WakerPoolInner {
    pool: FnvIndexMap<AppCookie, WakerRegistration, MAX_RECEIVERS>,
    next: usize,
}
    ...
    fn alloc(&self) -> Result<AppCookie> {
        self.inner.lock(|i| {
            ...
            loop {
                ...
                if let Entry::Vacant(e) = i.pool.entry(cookie) {
                    break if e.insert(WakerRegistration::new()).is_err() {
                        // Full
                        Err(Error::NoSpace)
                    } else {
                        Ok(cookie)
